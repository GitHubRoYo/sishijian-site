...
调用 src/lib/api.ts (统一接口)
     │
     └──→ src/lib/strapiApi.ts ──→ Strapi CMS (localhost:1337)
```

**当前状态：** 使用 Strapi 5 CMS
**CMS 后台：** http://localhost:1337/admin
...
├── src/lib/                         # 🔧 工具函数
│   ├── api.ts                       # ⭐ 统一数据接口（重要！）
│   ├── strapiApi.ts                 # Strapi REST API 适配器
│   ├── types.ts                     # CMS 类型定义
│   └── i18n.ts                      # 语言配置
│
├── .env                             # 🔐 环境配置（重要！）
...
1. 在 `src/lib/types.ts` 添加新的类型定义
2. 在 `src/lib/strapiApi.ts` 添加 API 调用函数
3. 在 `src/lib/api.ts` 导出新函数
4. 页面从 `@/lib/api` 导入使用
...
**看不到数据：**
- 检查 `.env` 中的 `NEXT_PUBLIC_STRAPI_URL` 设置
- 确认 Strapi CMS 后端正在运行 (http://localhost:1337)
- 查看浏览器控制台是否有错误提示
